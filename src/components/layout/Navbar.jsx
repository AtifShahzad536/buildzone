                ...
                return (
                  <div key={item.title} className="relative group/nav py-2">
                    ...
                    >
                      ...
                      {item.dropdown && (
                        <ChevronDown className="w-3.5 h-3.5 transition-transform duration-200 group-hover/nav:rotate-180 text-slate-400 group-hover/nav:text-[#0066FF]" />
                      )}
                    </Link>
                    ...
                    {item.dropdown && (
                      ...
                      >
                        ...
                        <div className="w-[580px] xl:w-[680px] bg-white border border-slate-200 shadow-2xl rounded-xl p-5 relative overflow-hidden transition-all duration-150 font-sans">
                          ...
                          <div className="flex items-center justify-between pb-3.5 mb-3.5 border-b border-slate-100">
                            ...
                            <Link 
                              to={item.href} 
                              className="font-sans text-[11px] font-bold uppercase tracking-wider text-slate-600 hover:text-[#0066FF] flex items-center gap-1"
                            >
                              <span>View All</span>
                          ...
                          <div className="grid grid-cols-3 gap-2.5">
                            {item.dropdown.map((subItem) => (
                              ...
                              >
                                <div>
                                  <div className="font-sans font-bold text-xs text-[#0B1938] group-hover/item:text-[#0066FF] transition-colors leading-snug">
                                    {subItem.title}
                                  </div>
                                  {subItem.desc && (
                          ...
                          <div className="mt-3.5 pt-3 border-t border-slate-100 flex items-center justify-between text-[11px] font-sans text-slate-600">
          ...
          <div className="p-4 sm:p-6 space-y-3">
            {siteConfig.navLinks.map((item) => (
              <div key={item.title} className="border border-slate-200 rounded-lg overflow-hidden bg-white">
                ...
                >
                  <span className="font-sans text-sm font-semibold text-[#0B1938]">
                    {item.title}
                  </span>
                  {item.dropdown && (
                    <ChevronDown
                  ...
                  <div className="p-2 bg-white border-t border-slate-200 space-y-1">
                    {item.dropdown.map((sub) => (
                      ...
                      >
                        ...
                        {sub.desc && <div className="text-[10.5px] text-slate-500">{sub.desc}</div>}
                      </Link>
                    ))}
                  </div>
                )}
            ...
            <div className="pt-4 border-t border-slate-200 text-center space-y-1">
              <p className="text-[12px] text-slate-700 font-sans font-medium">
              ...
              <p className="text-[12px] text-[#0B1938] font-sans font-bold pt-0.5">
